...
# signed fixed point exponent (i.e. leading '^')
def t_EXP(t):
    r'\^[+-]?\d+(\.5)?'
    fval = float(t.value[1:])
    t.value = int(fval) if fval.is_integer() else fval
    return t
